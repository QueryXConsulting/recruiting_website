...
const iterationObject = reactive([
    { required: true, prop: 'setIdentityCard', title: '身份证', multiple: true, limit: 2, accept: '.jpg,.jpeg,.png', tip: '请上传身份证正背面或户口照片' },
    ...
    { required: true, prop: 'setBankCard', title: '银行卡', multiple: true, limit: 2, accept: '.jpg,.jpeg,.png', tip: '请上传银行卡正背面照片' },
...
//     if (iterationObject[index].limit === files.length) {
        // console.log('文件数量超出限制', files);
        // const data = new FormData();
        // data.append(iterationObject[index].prop, files[0].raw)
        // data.append(iterationObject[index].prop, files[1].raw)
        // const res1 = await materialUpload(data);
        // if (!res1.content) {
        //     ElMessage.error(res1.message);
        //     return;
        // }
        ...
        <el-row justify="start">
            <el-col v-for="(item, index) in filteredIO" :key="index" :span="8" :gutter="50" class="upload-item">
                ...
                <el-upload v-model:file-list="fileList[index]" :accept="item.accept" :multiple="item.multiple" action="#"
                    ...
                    :auto-upload="false" :on-change="(file, files) => handleFileChange(index, files)">
